...
export class Instanced3Group extends THREE.Group {
    ...
    createVisibleInstancedBuffer(count){
        const ibAll=this.getInstancedBufferAll()
        for(let instancedBufferName in ibAll){
            const itemSize=ibAll[instancedBufferName].itemSize
            this["visible_"+instancedBufferName]=
                new THREE.InstancedBufferAttribute(new Float32Array(count*itemSize), itemSize)
        }                
    }
    ...
    createMeshAll(arr,meshType,meshTypeList,meshTypeListElem,cb){
        ...
        function next(i){
            ...
            if(meshType0==null){
                ...
                meshType0,
                scope.getInstanced0Shader(),
                m2=>{
                    scope.#children0[m2.meshTypeId]=m2
                    scope.add(m2)
                    indexFlag++
                    if(indexFlag==arr.length){
    ...
    getMeshByMeshTypeId(meshTypeId){
        return this.#children0[meshTypeId]
    }
    getCrowdPoints(){
    ...
    update(){
        ...
        for(let i=this.children.length-1;i>=0;i--){
            const mesh=this.children[i]
            mesh.visible=true
            this.updateMesh_all(mesh)//this.updateMesh_split(mesh)//
