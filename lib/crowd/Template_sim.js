...
import{Instanced0Shader}from './shaderSim/Instanced0Shader.js'
...
export class Template {
    constructor(scene, camera,poslist,config,materialLoaderType,cb) {
        ...
        this.init(config)
        // avatar.crowd.children[0].children[0].children[0].material
        
        // this.updateMap( "./assets/avatar/textures_sim1/man_linzhou/m2.jpg",1000)

    }
    updateMap(i5,url,time){
        const self=this
        setTimeout(()=>{
            console.log(self.crowd)
            const material=i5.children[0].children[0].children[0].material
            ...
            window.loadJson(path,data=>{
                ...
                for(let t in data){
                    ...
                    mesh.material.map=new THREE.TextureLoader().load( 
                            "./assets/avatar/textures_sim1/man_linzhou2/m2.webp"
                        )
    ...
    async loadMaterial_glb(path){//c.path+"sim.glb"
        return new Promise((resolve, reject) => {
            ...
            //                     "./assets/avatar/textures_sim1/man_linzhou/test.jpg"
            //                 )
            //             })
            ...
            mesh.name="business02_m_mediumpoly"
            mesh.skeleton={
                bones:{
                    length:34
                }
    ...
    async init(config) {
        ...
        function process(scenes){
            ...
            if(window.quality=="low")
            setTimeout(()=>{
               self.scene.add(crowd.CrowdPoints) 
            },1000)
            // self.updateMap(crowd,"./assets/avatar/textures_sim1/man_linzhou/m0.jpg",0)
            // self.updateMap(crowd,"./assets/avatar/textures_sim1/man_linzhou/m1.webp",100)
            // self.updateMap(crowd,"./assets/avatar/textures_sim1/man_linzhou/m2.webp",110)
