export class vs_anim_sim{
	...
	float getBoneScale(float bone) { // 身体形变
		...
		else if((5.<=b&&b<=7.)||(78.<=b&&b<=80.))return B;//膝盖
		else if(b==8.||b==81.)return C;//踝关节
		else if((9.<=b&&b<=10.)||(82.<=b&&b<=83.))return D;//脚趾

		else if(12.<=b&&b<=13.)return (A+B)/2.;//脊柱
		else if(b==14.)return (A+C)/2.;//胸部
		else if(15.<=b&&b<=16.)return (A+D)/2.;//脖子
		else if(17.<=b&&b<=18.)return (B+C)/2.;//头

		else if(b==19.||b==47.)return (B+D)/2.;//肩胛骨
		else if((20.<=b&&b<=22.)||(48.<=b&&b<=50.))return (B+C+D)/2.;//肩部
		else if((23.<=b&&b<=25.)||(51.<=b&&b<=53.))return (A+C+D)/2.;//眉毛
		else if(b==26.||b==54.)return (A+B+D)/2.;//手腕
		else if((27.<=b&&b<=46.)||(55.<=b&&b<=74.))return (A+B+C)/2.;//手
		else return 0.;
