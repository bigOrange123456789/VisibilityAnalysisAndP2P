...
class PlayerControl{
    constructor(camera,speed){
        ...
        this.frustum;
        var scope=this;
        this.controller.KeyboardMoveStep=speed;
        function tool(){
...
function PlayerControl0(camera){
    ...
    myMouseManager.dragMouse=function (dx,dy) {
        ...
        }else if(myMouseManager.button===1){//滚轮拖动
            if(scope.operating) {
                // scope.move(0, 0.015 * dy, 0.015 * dx);
            }
        }else{//右键拖动
            if(scope.operating) {
                // scope.move(0, 1.5 * dy, 1.5 * dx);
            }
        }
    }
...
function MouseManager(){
    ...
    this.onMouseDown=function( event ) {//鼠标移动事件监听
        ...
        //if(!document.getElementById("autoPath").stopFlag)
