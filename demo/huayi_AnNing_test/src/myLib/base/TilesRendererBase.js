...
export class TilesRendererBase {
	...
	fetchTileSet( url, fetchOptions, parent = null ) {
		// var new_url = url.slice(0,-4)+"zip"; 		// 加载zip格式
		var new_url = url;							// 加载json格式
		if(new_url.slice(-1)==="p"){
			return new Promise(function(resolve){
				var loader = new LoadingManager()
				new ZipLoader().load(new_url,()=>{
				},()=>{
				}).then( ( zip )=>{
					loader.setURLModifier( zip.urlResolver )
					return zip.find( /\.(json)$/i )
				}).then(fileUrl=>{
					new FileLoader(loader).load(fileUrl[0],json=>{
