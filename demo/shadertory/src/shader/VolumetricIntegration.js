const VolumetricIntegration=/* glsl */`
/* Hi there!
...
#ifdef D_DEMO_FREE
	//在高度雾的基础上应用噪音 // Apply noise on top of the height fog?
    #define D_FOG_NOISE 1.0

	//高度雾乘法器展示改进与新的积分公式 // Height fog multiplier to show off improvement with new integration formula
    #define D_STRONG_FOG 0.0

    //启用/禁用体积阴影（单一散射阴影）。// Enable/disable volumetric shadow (single scattering shadow)
    #define D_VOLUME_SHADOW_ENABLE 1

	//是否使用放大的散射？// Use imporved scattering?
...
//在墙壁上应用凹凸贴图 // Apply bump mapping on walls
#define D_DETAILED_WALLS 0

//用来限制射线行进的长度。需要用于体积计算 // Use to restrict ray marching length. Needed for volumetric evaluation.
#define D_MAX_STEP_LENGTH_ENABLE 1

//光源的颜色和位置 // Light position and color
#define LPOS vec3( 20.0+15.0*sin(iTime), 15.0+12.0*cos(iTime)-5.5,-20.0)
#define LCOL (5.*600.0*vec3( 1.0, 0.9, 0.5)*(.8*cos(iTime)+.7))
...
{
    float heightFog = 7.0 + 5.*D_FOG_NOISE*3.0*clamp(displacementSimple(pos.xz*0.005 + iTime*0.01),0.0,1.0);
...
void traceScene(bool improvedScattering, vec3 rO, vec3 rD, inout vec3 finalPos, inout vec3 normal, inout vec3 albedo, inout vec4 scatTrans)
{
	...
	for(int i=0; i<numIter;++i)
	{
		...
		else
        {
            ...
            transmittance *= exp(-sigmaE * dd);
        #endif
            scatteredLight += sigmaS * evaluateLight(p) * phaseFunction() * transmittance * dd;
        #if !D_UPDATE_TRANS_FIRST
            transmittance *= exp(-sigmaE * dd);
        #endif
        }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 uv = fragCoord.xy / iResolution.xy;//转换为[ − 1 , 1 ]
	...
	vec3 rO = camPos;//射线的初始点
	vec3 rD = normalize(uv2.x*camX + uv2.y*camY + camZ);//射线的方向
	vec3 finalPos = rO;
