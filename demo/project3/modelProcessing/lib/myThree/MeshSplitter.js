...
const BufferGeometryEx=Engine3D.THREE_EX.BufferGeometryEx
...
class MeshSplitter{
    constructor(gltfScene,structDescription,matrixDescription,projectName){
        ...
        ipcRenderer.send("createDir", this.projectName+"\\input\\model");
        ipcRenderer.send("createDir", this.projectName+"\\input\\matrix");

        this.sumSplitted = 0;
        for(let i=0; i<this.structDescription.length; i++){
            this.sumSplitted += this.structDescription[i].length;
        }
        this.sumSplitted = Math.ceil(this.sumSplitted/300);
    ...
    splitAverage(index){
        ipcRenderer.send("console", "Calculate occluder ... "+index.toString() + "/"+this.sumSplitted.toString() + "\r");
        const group_num = 300
        ...
        while(N+this.structDescription[start_i].length <= start){
            ...
            if(start_i >= this.structDescription.length){
                ...
                ipcRenderer.send("console", "                                       \r");
                ipcRenderer.send("console", "Calculation completed.");
                new SpacePartitioning(this.newMeshList, this.structDescription, this.matrixDescription, this.projectName)
                return
            }
        }
        ...
        for(let n=0; n<group_num; n++,start_j++){
            ...
            if(this.structDescription[start_i].length===1){
                ...
                else object.geometry.computeVertexNormals()
        ...
        var fileData = JSON.stringify({
            name: this.projectName+"\\assets\\occluder\\occluder"+index.toString()+".json",
            data: JSON.stringify({
