...
class ViewpointList: #所有视点,每个视点的可见特征
    def __init__(self,config,getWall,path_pre_in,featureCoef):
        ...
        if getWall:
            ...
            # self.similarity_data2=self.init_similarity_data2()#没什么用
    ...
    def initFeatureSgn(self,featureAll1):
        featureAll2={}
        for idv in featureAll1:
            feature1=featureAll1[idv]
            feature2=[]
            for d in feature1:
                if d>0:feature2.append(1)
                else  :feature2.append(0)
            featureAll2[idv]=feature2
        return featureAll2
        ...
        for cid in range(cidMax+1):#遍历所有构件
            ...
            for vid in self.data:
                ...
                if str(cid) in d:#如果视点vid能够看到构件cid
                    if not d[str(cid)]==0:
                        if self.data[vid].getEntropy1()>threshold["threshold_entropy"]:#1.7:#熵过小的视点很可能是构件内部的视点
                            vidList.append(vid)
            for i in range(len(vidList)):
                name1=vidList[i].split(",")
                p1=[float(name1[0]),float(name1[1]),float(name1[2])]
                for j in range(len(vidList)):
                    name2=vidList[j].split(",")
                    p2=[float(name2[0]),float(name2[1]),float(name2[2])]
                    distance_square=(p1[0]-p2[0])**2+(p1[1]-p2[1])**2+(p1[2]-p2[2])**2
                    # print(distance_square)
                    if (not i==j) and distance_square< (threshold["threshold_distance"]**2):
                        s=self.similarity1(vidList[i],vidList[j])
                        # print(s)
                        if s<threshold["threshold_sim"]:iswall[cid]=True
        print("cidMax",cidMax)
