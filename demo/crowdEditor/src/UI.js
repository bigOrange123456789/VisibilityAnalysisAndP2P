...
export class UI{
    ...
    init(main){
        ...
        this.control_avatar(window.crowd)
    ...
    control_avatar(i5) {
        ...
        const params={
            ...
            '头部':i5.getBodyScale(0)[0],
            '手臂':i5.getBodyScale(0)[1],
            '肚子':i5.getBodyScale(0)[2],
            '腿部':i5.getBodyScale(0)[3],


            // '躯体类型':i5.getBodyScale(0)[0],
            '上衣':i5.getPartType(0,"coat"),
            '裤子':i5.getPartType(0,"trousers"),
            '头发':i5.getPartType(0,"hair"),
            '动画类型':i5.getAnimation(0)[0],
            '播放速度':i5.getSpeed(0),
        }
        const gui=this.gui
        const folder = gui.addFolder("化身编辑")
        folder.add(params,'身高', 0.5, 2.5).step( 0.005 ).onChange((value)=>{
            const i=window.avatarI
            console.log(window.avatarI)
            const s=i5.getScale(i)
            i5.setScale(i,[
                s[0],Number( value ),s[2]
            ])
            i5.update()
        })
        
        folder.add(params,'头部', -0.5, 0.5).step( 0.005 ).onChange((value)=>{
            const i=window.avatarI
            const s=i5.getBodyScale(i)
            i5.setBodyScale(i,[
                Number( value ),s[1],s[2],s[3]
            ])
            i5.update()
        })
        folder.add(params,'手臂', -0.5, 0.5).step( 0.005 ).onChange((value)=>{
            const i=window.avatarI
            const s=i5.getBodyScale(i)
            i5.setBodyScale(i,[
                s[0],Number( value ),s[2],s[3]
            ])
            i5.update()
        })
        folder.add(params,'肚子', -0.5, 0.5).step( 0.005 ).onChange((value)=>{
            const i=window.avatarI
            const s=i5.getBodyScale(i)
            i5.setBodyScale(i,[
                s[0],s[1],Number( value ),s[3]
            ])
            i5.update()
        })
        folder.add(params,'腿部', -0.5, 0.5).step( 0.005 ).onChange((value)=>{
            const i=window.avatarI
            const s=i5.getBodyScale(i)
            i5.setBodyScale(i,[
                s[0],s[1],s[2],Number( value )
            ])
            i5.update()
        })

        // body coat hair head trousers
        const folder2 = gui.addFolder("服装和发型")
        folder2.add( params, '上衣', {
            'A类': 0,
            'B类': 1,
            'C类': 2
        } ).onChange( function ( value ) {
            const i=window.avatarI
            i5.setPartType(i,"coat",parseInt( value ))
            i5.setPartType(i,"body",parseInt( value ))
            i5.update()
        } );
        folder2.add( params, '裤子', {
            'A类': 0,
            'B类': 1,
            'C类': 2
        } ).onChange( function ( value ) {
            const i=window.avatarI
            i5.setPartType(i,"trousers",parseInt( value ))
            i5.setPartType(i,"body",parseInt( value ))
            i5.update()
        } );
        folder2.add( params, '头发', {
            '短发': 0,
            '长发': 1,
            '散发': 2
        } ).onChange( function ( value ) {
            const i=window.avatarI
            i5.setPartType(i,"hair",parseInt( value ))
            i5.update()
        } );

        folder2.addColor({'上衣颜色':new THREE.Color(0xffffff)}, '上衣颜色')
        .onChange(function(e)
        {
            const i=window.avatarI
            let j=10
            crowd.setColor(i,[j*e.r*2,j*e.g,j*e.b],"CloW_A_waitao_geo1")

            j=-0.3*10
            crowd.setColor(i,[j*(1-e.r),j*(1-e.g),j*(1-e.b)],"waitao1")

            // j=10//0.1
            crowd.setColor(i,[j*e.r,j*e.g,j*e.b],"CloW_C_shangyi_geo")
            i5.update()
        });
        folder2.addColor({'裤子颜色':new THREE.Color(0xffffff)}, '裤子颜色')
        .onChange(function(e)
        {
            const i=window.avatarI
            let j=10
            crowd.setColor(i,[j*e.r*2,j*e.g,j*e.b],"CloW_A_kuzi_geo")

            j=-0.3*10
            crowd.setColor(i,[j*(1-e.r),j*(1-e.g),j*(1-e.b)],"qipao22")

            j=0.1
            crowd.setColor(i,[j*e.r,j*e.g,j*e.b],"CloW_C_qunzi_geo3456")
            i5.update()
        });
        folder2.addColor({'鞋子颜色':new THREE.Color(0xffffff)}, '鞋子颜色')
        .onChange(function(e)
        {
            const i=window.avatarI
            let j=10
            i5.setColor(i,[j*e.r*2,j*e.g,j*e.b],"CloW_A_xiezi_geo")
            i5.update()
        });
        


        const folder3 = gui.addFolder("动画")
        folder3.add( params, '动画类型', {
            'stand0': 0,
            'stand1': 1,
            'stand2': 2,
            'stand3': 3,
        ...
        folder3.add( params, '播放速度',0, 30).step( 0.1 ).onChange( function ( value ) {
            const i=window.avatarI
            i5.setSpeed(
                i,
                value 
        ...
        const folder4 = gui.addFolder("渐进式三角网格")
        folder4.add({'三角网格精细度':0.7},'三角网格精细度', 0.05, 1.0).step( 0.05 ).onChange((value)=>{
            const lod=parseInt(value/0.05)
            i5.useLod(lod);
        })
