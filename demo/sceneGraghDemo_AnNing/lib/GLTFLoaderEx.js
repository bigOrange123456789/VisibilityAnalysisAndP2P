...
class GLTFLoaderEx extends Loader {
	constructor(manager) {
		super(manager);
...
class GLTFParser {
	...
	loadMaterial(materialIndex) {
		...
		if (
			materialDef.alphaTexture !== undefined &&
			materialType !== MeshBasicMaterial
		) {
			pending.push(
				parser.assignTexture(
	...
	loadMesh(meshIndex) {
		...
		return Promise.all(pending).then(function (results) {
			...
			//TKB
			if (meshDef.extras && meshDef.extras.uuid) {
				meshes[0].uuid = meshDef.extras.uuid;
			}
	...
	loadNode(nodeIndex) {
		...
		return (function () {
			...
			//TKB
			if (nodeDef.extras && nodeDef.extras.uuid) {
				node.uuid = nodeDef.extras.uuid;
			}
	...
	loadScene(sceneIndex) {
		...
		//TKB
		if (sceneDef.extras && sceneDef.extras.uuid) {
			scene.uuid = sceneDef.extras.uuid;
		}
