...
//import { GLTFLoaderEx } from "../../../../lib/three_electron/examples/jsm/loaders/GLTFLoaderEx.js";
// import { GLTFExporter } from "../../../../lib/three_electron/examples/jsm/exporters/GLTFExporter.js";
import {Engine3D} from"../../src/main.js"
const GLTFExporter=Engine3D.GLTFExporter
...
export class SpacePartitioning {
  constructor(meshList, structDesc, matrixDesc, projectName) {
    ...
    // download(structDesc, this.projectName+"\\structdesc.json");
    // download(matrixDesc, this.projectName+"\\smatrix.json");
  ...
  buildTree() {
    ...
    download(scene_graph, this.projectName+"\\assets\\sceneGraph.json");

    var self = this;
    // setTimeout(function () {
    download(instanceSphereList, this.projectName+"\\assets\\boundingSphere.json");
    //  }, 200);

    // setTimeout(function () {
    download(instanceBoxList, this.projectName+"\\assets\\boundingBox.json");
    //  }, 400);

    ipcRenderer.send("console", "SceneGraph calculation completed.");
    ipcRenderer.send("console", "Model exporting ... ");
  ...
  async exportGLTF() {
    ...
    let exportFun = (index) => {
      ...
      group.add(object);
      scene.add(group);
      let fileName = this.projectName+"\\input\\model\\model" + index + ".gltf";
      new GLTFExporter().parse(scene, async function (result) {
        ...
        if (index === export_end) {
          self.exportJSON();
          ipcRenderer.send("console", "Model export completed.");
  ...
  exportJSON(){
    ...
    for(let i=0; i<this.structList.length; i++)
      matrix_list.push(this.matrixList[this.structList[i].n].it);

    var self = this;
    let exportMat = async function (index) {
      let fileName = self.projectName+"\\input\\matrix\\matrix"+index+".json";

      var fileData = JSON.stringify({
        name: fileName,
        data: JSON.stringify(matrix_list[index])
      });
      await ipcRenderer.invoke("exportJSON", fileData);
      index++;
      if (index === matrix_list.length) {
        console.log("export end");
        ipcRenderer.send("console", "File compressing ...");
        self.exportZIP();
        // ipcRenderer.send("quit", "export end");
      } else {
        exportMat(index);
  ...
  exportZIP(){
    var self = this;
    let Compress = async function (index) {

      var fileData = JSON.stringify({
        name: self.projectName,
        index: index
      })
      await ipcRenderer.invoke("compressFiles", fileData);
      index++;
      if (index === self.structList.length) {
        console.log("compress end");
        ipcRenderer.send("console", "Files compression completed.");
        ipcRenderer.send("quit", self.projectName);
      } else {
        setTimeout(function(){
          Compress(index);
        }, 30);
