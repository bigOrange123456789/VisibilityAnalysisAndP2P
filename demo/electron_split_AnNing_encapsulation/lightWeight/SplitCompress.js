...
class SplitCompress{
    constructor(opt){
        if(!opt)opt={}
        opt.inpath=opt.inpath?opt.inpath:"../../../dist/assets/project1/temp"
        opt.outpath=opt.outpath?opt.outpath:"../../../dist/assets/project1"
        this.opt=opt
        this.start = 0
        this.matrix_list = []
        this.end
    ...
    _compress(){
        ...
        var processing = setInterval(()=>{
            ...
            self._zip(i)
            if(++i===self.end) clearInterval(processing)
        },50)
    }
    _zip(index){
