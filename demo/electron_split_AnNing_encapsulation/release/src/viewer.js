...
export class Viewer {
  constructor(el, options) {
    ...
    this.addGUI();
    if (window.projectName === "LS_HULL_EDIS") {
      this.defaultCamera.up.set(0, 0, 1);
      this.addMyUI();
    }
    ...
    const self=this
    this.getCubeMapTexture('assets/environment/evn.jpg').then(
  ...
  setCamera() {
    ...
    if (window.projectName === "LS_HULL_EDIS") {
      this.defaultCamera.position.set(0, -1800, 550);
      this.defaultCamera.lookAt(0, -1050, -100);
      this.orbitControl.target.set(0, -1050, -100);
    }
    if (window.projectName === "nanjing") {
      this.defaultCamera.position.set(0, -1000, 2000);
      this.defaultCamera.lookAt(0, 0, 0);
