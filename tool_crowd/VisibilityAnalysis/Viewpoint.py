class Viewpoint:#单个视点的可见度信息
    ...
    def load3(self,path):
        arr=path.split("/")
        self.name=arr[len(arr)-1].split(".json")[0]
        def test(d):
            if d==None:
                return {}
            d2={}
        ...
        with open(path) as f:
            ...
            self.data[str(1+1)]=test(json_data[0])
            self.data[str(1+0)]=test(json_data[1])

            self.data[str(1+3)]=test(json_data[2])
            self.data[str(1+2)]=test(json_data[3])

            self.data[str(1+5)]=test(json_data[4])
            self.data[str(1+4)]=test(json_data[5])
