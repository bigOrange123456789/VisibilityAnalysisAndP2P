...
public:
    ...
    {
        Box model_box = meshes[0].computeBoudningBox();
        for(unsigned int i = 1; i < meshes.size(); i++)
        {
            Box mesh_box = meshes[i].computeBoudningBox();
            model_box.adjustBox(mesh_box);
            meshes[i].computeBoudningSphere();
        }
        return model_box;
    }
...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        // aiString mName = mesh->mName;
        // name = mName.C_Str();
        // cout<<mName.C_Str()<<endl;
        ...
        // read material while the mesh has no texture
        aiColor3D color;
